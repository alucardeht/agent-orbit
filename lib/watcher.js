    ...
    if (target.enabled) {
      ...
      if (paths) {
        ...
        if (paths.config && fs.existsSync(paths.config)) {
          pathsToWatch.push(paths.config);
        }

        // Add mcp config file
        if (paths.mcpConfig && fs.existsSync(paths.mcpConfig)) {
          pathsToWatch.push(paths.mcpConfig);
        }
      }
    }
  });
  ...
  watcher.on('all', (event, filePath) => {
    ...
    for (const [agent, paths] of Object.entries(activeAgentPaths)) {
      if (filePath.startsWith(paths.home) || 
         (paths.config && filePath.startsWith(path.dirname(paths.config))) ||
