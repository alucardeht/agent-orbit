...
function runDaemon() {
  ...
  try {
    ...
    // Keep the Node event loop alive indefinitely in case chokidar's handles are unreferenced or garbage-collected
    setInterval(() => {
      // Dummy timer to prevent exit
    }, 1000 * 60 * 60 * 24); // 24 hours tick
  } catch (err) {
