...
class GitManager {
  ...
  async _pushWithPullRetry() {
    try {
      ...
      await this.pullWithStrategy('ours');
      await this._retryWithBackoff(
        () => this.git.push('origin', 'main'),
    ...
    return lock.withLock(async () => {
      ...
      try {
        ...
        try {
          ...
          for (const dir of ['codex', 'claude', 'antigravity', 'kimi']) {
  ...
  async pullWithStrategy(strategyName = 'theirs', gitInstance = null) {
    return lock.withLock(async () => {
      ...
      try {
        ...
        const msg = pullError.message || String(pullError);

        // Fallback: If Git is still stuck in an unresolvable logical merge block
        if (msg.includes('CONFLICT') || msg.includes('rebase') || msg.includes('could not apply') || msg.includes('Merge conflict')) {
