...
function loadConfig() {
  ...
  try {
    ...
    // Auto-migrate missing targets from updates
    let configUpdated = false;
    if (!userConfig.targets) {
      userConfig.targets = defaultConfig.targets;
      configUpdated = true;
    } else {
      Object.keys(defaultConfig.targets).forEach(target => {
        if (!userConfig.targets[target]) {
          userConfig.targets[target] = defaultConfig.targets[target];
          configUpdated = true;
        }
      });
    }
