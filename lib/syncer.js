...
function syncAll(config, repoPath, options = {}) {
  ...
  Object.keys(config.targets).forEach(agentName => {
    ...
    // 5. Copy Config file
    if (paths.config && fs.existsSync(paths.config) && target.syncConfig) {
      const destFile = path.join(agentRepoDir, path.basename(paths.config));
      copyFileSync(paths.config, destFile);
      changed = true;
    }

    // 6. Copy Antigravity MCP Config
    if (paths.mcpConfig && fs.existsSync(paths.mcpConfig) && target.syncConfig) {
      const destFile = path.join(agentRepoDir, path.basename(paths.mcpConfig));
      copyFileSync(paths.mcpConfig, destFile);
      changed = true;
    }
  });
  ...
  return pullFromRemote(git)
    .then(() => {
      Object.keys(config.targets).forEach(agentName => {
        ...
        if (paths.config && target.syncConfig) {
          const repoFile = path.join(agentRepoDir, path.basename(paths.config));
          if (fs.existsSync(repoFile)) {
            copyFileSync(repoFile, paths.config);
          }
        }

        // 6. Restore Antigravity MCP config
        if (paths.mcpConfig && target.syncConfig) {
          const repoFile = path.join(agentRepoDir, path.basename(paths.mcpConfig));
          if (fs.existsSync(repoFile)) {
            copyFileSync(repoFile, paths.mcpConfig);
          }
        }
      });
