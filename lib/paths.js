...
function getAgentPaths(agentName, targetConfig) {
  ...
  if (agentName === 'codex') {
    ...
    return {
      home,
      rules: path.join(configHome, 'antigravity_rules.md'),
      skillsDir: path.join(home, 'skills'),
      config: path.join(configHome, 'config.json'),
      mcpConfig: path.join(configHome, 'mcp_config.json')
    };
    ...
    return {
      ...
      config: path.join(actualHome, 'config.toml')
    };
  }
  return null;
}
